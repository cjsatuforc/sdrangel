	...
	if (deviceOk && (doLPCalibration || doChangeSampleRate))
    {
        ...
        if (doCalibration)
        {
            if (LMS_Calibrate(m_deviceShared.m_deviceParams->getDevice(),
                              ...
                              0) < 0)
            {
                qCritical("XTRXInput::applySettings: calibration failed on Rx channel %d", m_deviceShared.m_channel);
            }
            else
            {
                qDebug("XTRXInput::applySettings: calibration successful on Rx channel %d", m_deviceShared.m_channel);
            }
        }
#endif
        if (doLPCalibration)
